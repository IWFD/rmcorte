<?php
/**
 * Função relacionada a funções de autenticação & nonces
 */


/**
 * Exibe o bloco de login se necessário.
 *
 * @return void
 */
...
function yourls_is_valid_user() {
	...
	if( isset( $_GET['action'] ) && $_GET['action'] == 'logout' && isset( $_REQUEST['nonce'] ) ) {
		...
		yourls_store_cookie( '' );
		return yourls__( 'Deslogado com sucesso' );
	}
	...
	if ( isset( $_REQUEST['username'] ) || isset( $_REQUEST['password'] ) ) {
		return yourls__( 'Usuário Inválido ou Senha' );
	} else {
		return yourls__( 'Por favor, faça o login no sistema.' );
	}
}
...
function yourls_hash_passwords_now( $config_file ) {
	if( !is_readable( $config_file ) ) {
        yourls_debug_log( 'Não é possível fazer hash de senhas: não é possível ler o arquivo ' . $config_file );
	...
	foreach ( $yourls_user_passwords as $user => $password ) {
		...
		if ( !yourls_has_phpass_password( $user ) && !yourls_has_md5_password( $user ) ) {
			...
			$replace = "'$user' => 'phpass:$hash' /* Senha encriptada por RMCorte */ ";
			$count = 0;
...
function yourls_store_cookie( $user = '' ) {
    ...
    if ( !headers_sent( $filename, $linenum ) ) {
        ...
        yourls_debug_log( "Não foi possível armazenar o cookie: cabeçalhos já enviados $filename on line $linenum" );
	}
}
...
function yourls_verify_nonce($action, $nonce = false, $user = false, $return = '' ) {
	...
	if( $nonce === $valid ) {
		...
		if( $return )
			die( $return );
		yourls_die( yourls__( 'Ação não autorizada ou link expirado' ), yourls__( 'Error' ), 403 );
	}
}
